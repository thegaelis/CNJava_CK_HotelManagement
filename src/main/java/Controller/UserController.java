...
public class UserController extends HttpServlet {
	...
	protected void addUser(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(name.trim().isEmpty() || age.trim().isEmpty()|| salary.trim().isEmpty()|| email.trim().isEmpty()|| password.trim().isEmpty() ) {
			request.setAttribute("mess", "Please fill input");
			...
			if (users.stream().anyMatch(x -> x.getEmail().equals(email))) {
				request.setAttribute("mess", "Email has exits");
				doGet(request,response);
	        }
			else {
				...
				if(add) {
					request.setAttribute("mess", "Add user succesfull");
					doGet(request,response);
				}
				else {
	...
	protected void updateUser(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(name.trim().isEmpty() || age.trim().isEmpty()|| salary.trim().isEmpty()|| email.trim().isEmpty()|| password.trim().isEmpty() ) {
			...
			if (users.stream().anyMatch(x -> (x.getEmail().equals(email)))) {
				...
				if(update) {
					request.setAttribute("mess", "Update user succesfull");
					doGet(request,response);
				}
				else {
					request.setAttribute("mess",null);
	...
	protected void deleteUser(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(delete) {
			request.setAttribute("mess", "Delete user succesfull");
			doGet(request,response);
		}
		else {
			request.setAttribute("mess",null);
